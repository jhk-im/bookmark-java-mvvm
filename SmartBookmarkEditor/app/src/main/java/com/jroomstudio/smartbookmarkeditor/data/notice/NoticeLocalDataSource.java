...
public class NoticeLocalDataSource implements NoticeDataSource {

    // 인스턴스
    // 해당 변수는 Main Memory 에 저장
    private static volatile NoticeLocalDataSource INSTANCE;
    ...
    // 다이렉트 인스턴스 방지
    // 다이렉트 인스턴스 방지
    private NoticeLocalDataSource(@NonNull AppExecutors appExecutors,
    ...
    // 싱글 인스턴스 리턴
    public static NoticeLocalDataSource getInstance(@NonNull AppExecutors appExecutors,
                                                    @NonNull NotificationsDAO notificationsDAO){
        if(INSTANCE == null){
            synchronized (NoticeLocalDataSource.class){
                if(INSTANCE == null){
                    INSTANCE = new NoticeLocalDataSource(appExecutors, notificationsDAO);
                }
            }
        }
        return INSTANCE;
