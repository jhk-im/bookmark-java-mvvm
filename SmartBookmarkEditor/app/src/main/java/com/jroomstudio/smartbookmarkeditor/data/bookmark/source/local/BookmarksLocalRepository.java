 ...
 **/
public class BookmarksLocalRepository implements BookmarksLocalDataSource {

    /**
     * - BookmarksLocalDataSource 의 INSTANCE
     * - volatile 은 Java 변수를 Main memory 에 저장하겠다는 것을 명시하는 것
     **/
    private static volatile BookmarksLocalRepository INSTANCE;
    ...
    // 다이렉트 인스턴스 방지
    private BookmarksLocalRepository(@NonNull AppExecutors appExecutors,
     ...
     **/
    public static BookmarksLocalRepository getInstance(@NonNull AppExecutors appExecutors,
                                                       @NonNull BookmarksDAO bookmarksDAO){
        if(INSTANCE == null){
            synchronized (BookmarksLocalRepository.class) {
                if(INSTANCE == null){
                    INSTANCE = new BookmarksLocalRepository(appExecutors, bookmarksDAO);
                }
            }
        }
        return INSTANCE;
    ...
    /*
     * BookmarksLocalDataSource 오버라이드 메소드 구현
     */
